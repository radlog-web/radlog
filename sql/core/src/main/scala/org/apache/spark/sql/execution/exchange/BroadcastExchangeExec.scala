  ...
  private lazy val relationFuture: Future[broadcast.Broadcast[Any]] = {
    ...
    Future {
      ...
      SQLExecution.withExecutionId(sparkContext, executionId) {
        ...
        val relation = mode match {
          case _: PackedIdentityBroadcastMode =>
            // we skip mode.transform(input) as we let workers transform the internal row to bytes
            val start = System.currentTimeMillis()
            val input: Array[Array[Byte]] = PackedBroadcastUtil.getByteArrayRdd(child).collect()
            logInfo(s"PackedIdentityBroadcastMode - child.getByteArrayRdd.collect()" +
              s" took: ${System.currentTimeMillis() - start} ms")
            input
          case _ =>
            ...
            val res = mode.transform(input)
            longMetric("buildTime") += (System.nanoTime() - beforeBuild) / 1000000
            res
        }
