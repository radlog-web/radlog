    ...
    left.execute().zipPartitions(right.execute()) { (leftIter, rightIter) =>
      ...
      joinType match {
        ...
        case FullOuter =>
          val leftNullRow = new GenericInternalRow(left.output.length)
          val rightNullRow = new GenericInternalRow(right.output.length)
          // Youfu Li added this two lines to support fill the Cy and Cn with 0 in the fullouter join
          leftNullRow.setValue(left.output.length - 1, 1)
          rightNullRow.setValue(right.output.length - 1, 1)
