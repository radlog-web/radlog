...
import org.apache.spark.internal.config.{CATALOG_IMPLEMENTATION, SESSION_STATE}
...
object SparkSession {
  ...
  private def sessionStateClassName(conf: SparkConf): String = {
    // TODO: update sharedStateClassName as well
    conf.get(SESSION_STATE) match {
      case "rasql" => "edu.ucla.cs.wis.bigdatalog.spark.BigDatalogSessionState"
      case _ =>
        conf.get(CATALOG_IMPLEMENTATION) match {
