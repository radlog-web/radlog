...
class SchemaInfo(val schema: StructType, val keyPositions: Array[Int])
  ...
  val bytesPerKey: Int = {
    ...
    for (i <- 0 until schema.length)
      if (keyPositions(i) == 1) bytesPerKey += schema(i).dataType.defaultSize
    bytesPerKey
  }
  ...
  def bytesPerValue: Int = {
    ...
    for (i <- 0 until schema.length)
      if (keyPositions(i) == 0) bytesPerValue += schema(i).dataType.defaultSize
    bytesPerValue
  }
  ...
  def keyColumns: StructType = {
    ...
    for (i <- 0 until schema.length)
      if (keyPositions(i) == 1) keyColumns += schema(i)
  ...
  def valueColumns: StructType = {
    ...
    for (i <- 0 until schema.length)
      if (keyPositions(i) == 0) valueColumns += schema(i)
...
class NestedSchemaInfo(schema: StructType, keyPositions: Array[Int], subKeyPositions: Array[Int])
  ...
  override def valueIndexes: Array[Int] = {
    ...
    for (i <- 0 until subKeyPositions.length)
      if (subKeyPositions(i) == 1) allKeyPositions(i) = 1
  ...
  override def bytesPerValue: Int = {
    var bytesPerValue: Int = 0
    for (i <- 0 until schema.length)
      if (keyPositions(i) == 0 && subKeyPositions(i) == 0) bytesPerValue += schema(i).dataType.defaultSize
    bytesPerValue
  }
  ...
  def subKeyColumns: StructType = {
    ...
    for (i <- 0 until schema.length)
      if (subKeyPositions(i) == 1) subKeyColumns += schema(i)
  ...
  override def valueColumns: StructType = {
    val valueColumns = new ArrayBuffer[StructField]
    for (i <- 0 until schema.length)
      if (keyPositions(i) == 0 && subKeyPositions(i) == 0) valueColumns += schema(i)
    StructType(valueColumns.toArray)
  }
}
