...
import org.apache.spark.sql.datalog.LazyUtils
...
trait HashJoin {
  ...
  private def innerJoin(
    ...
    val ret = streamIter.flatMap { srow =>
      joinRow.withLeft(srow)
      val matches = hashedRelation.get(joinKeys(srow))
      // println("srow: " + srow + " matched? " + ((matches != null) && matches.nonEmpty))
      if (matches != null) {
  ...
  protected def join(
    ...
    val before = numOutputRows.value
    val res = joinedIter.map { r =>
      numOutputRows += 1
      // for debug
      // val ret = resultProj(r)
      // println("Joined: " + ret)
      // ret
      resultProj(r)
    }
    val count = numOutputRows.value - before
    // logWarning("Joined Row Count: " + count)
    res
  }
}
