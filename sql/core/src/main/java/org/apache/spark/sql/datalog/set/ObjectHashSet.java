...
public class ObjectHashSet extends it.unimi.dsi.fastutil.objects.ObjectOpenHashSet<InternalRow>
    implements HashSet {
    ...
    public void ifNotExistsInsert(InternalRow row, HashSet diffSet) {
        if (!this.contains(row))
            ((ObjectHashSet)diffSet).add(row.copy());
    }

    public HashSet union(HashSet other) {
        super.addAll((ObjectHashSet)other);
        return this;
    }
