...
public final class UnsafeExternalRowSorter {
  ...
  public abstract static class PrefixComputer {
    public abstract long computePrefix(InternalRow row);
  }
  ...
  // [Hack] victor: for sorter reuse
  public boolean sorted = false;

  // victor: we do not cleanupResources in sortReuse
  private Iterator<UnsafeRow> sortReuse(boolean sorted) throws IOException {
    try {
      final UnsafeSorterIterator sortedIterator = sorter.getSortedIterator(sorted);
      return new AbstractScalaRowIterator<UnsafeRow>() {
  ...
  public Iterator<UnsafeRow> sortReuseInit(Iterator<UnsafeRow> inputIterator) throws IOException {
    while (inputIterator.hasNext()) {
      insertRow(inputIterator.next());
    }
    Iterator<UnsafeRow> iter = sortReuse(false);
    sorted = true;
    return iter;
  }

  public Iterator<UnsafeRow> sortReuse() throws IOException {
    return sortReuse(true);
  }
