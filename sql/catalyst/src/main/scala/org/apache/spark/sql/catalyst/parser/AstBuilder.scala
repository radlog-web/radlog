...
class AstBuilder extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  override def visitQuery(ctx: QueryContext): LogicalPlan = withOrigin(ctx) {
    ...
    if (ctx.rec == null) {
      ...
      // victor: replace cte by rec
      val viewDefs = ctx.rec.viewDef.asScala.map { vCtx =>
        visitViewDef(vCtx)
      }
      // Check for duplicate names.
      checkDuplicateKeys(viewDefs, ctx)
      WithRec(query, viewDefs.toMap, source(ctx))
    }
  }

  // victor
  override def visitViewDef(ctx: ViewDefContext): (String, (Seq[Expression], LogicalPlan)) =
    withOrigin(ctx) {
      (ctx.name.getText, (visitTableCols(ctx.tableCols), plan(ctx.queryTerm)))
    }

  // victor: similar to visitNamedExpression
  override def visitTableCols(ctx: TableColsContext): Seq[Expression] = {
    ctx.col.asScala.map { ctx =>
