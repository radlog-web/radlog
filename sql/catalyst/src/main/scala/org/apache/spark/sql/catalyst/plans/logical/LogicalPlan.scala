...
abstract class LogicalPlan extends QueryPlan[LogicalPlan] with Logging {
  ...
  private def resolveAsColumn(
      ...
      resolver: Resolver,
      attribute: Attribute): Option[(Attribute, List[String])] = {
    val disCntList = Array("Var_60", "Var_92", "Var_134", "Var_164", "Var_180", "Var_20", "Var_13")
    if (!attribute.isGenerated && resolver(attribute.name, nameParts.head)) {
      Option((attribute.withName(nameParts.head), nameParts.tail.toList))
    } else if (!attribute.isGenerated && attribute.qualifier.exists(attr =>
      SparkEnv.getUnionSet().exists(union => resolver(attr, union)))
      && attribute.name.contains(nameParts.head)) {
      // This is a hack by Youfu Li, to let the system be able to resolve attributes in Union.
      Option((attribute.withName(nameParts.head), nameParts.tail.toList))
    } else if (!attribute.isGenerated && nameParts.size > 1
      && resolver(attribute.name, nameParts.toList(1))) {
      // to deal with the situation that nameParts.size > 1
      Option((attribute.withName(nameParts.toList(1)), Nil))
    } else if (!attribute.isGenerated && attribute.name.contains(nameParts.head)) {
      // deal with situation that the attribute.name is end with numbers, such as Tid36, etc.
      Option((attribute.withName(nameParts.head), nameParts.tail.toList))
    } /* else if ((nameParts.head.contains("Var_47") || (nameParts.head.contains("Var_49") || nameParts.head.contains("Var_42")) // Var_54
      && attribute.name.contains("Var_12") // Var_12
      && attribute.qualifier.get.equals("cntdec"))) {
      Option((attribute.withName(nameParts.head), nameParts.tail.toList))
    } */ else if ((nameParts.head.contains("Var_31") || nameParts.head.contains("Var_39"))
      && attribute.name.contains("Var_12") // Var_12
      && attribute.qualifier.get.equals("cntdec")) {
      Option((attribute.withName(nameParts.head), nameParts.tail.toList))
    } else if (nameParts.head.contains("DisCnt")
      && disCntList.exists(attr => attribute.name.contains(attr))
      && attribute.qualifier.get.equals("distc")) {
      Option((attribute.withName(nameParts.head), nameParts.tail.toList))
    } else {
      None
    }
  ...
  protected def resolve(
    ...
    candidates.distinct match {
      ...
      case ambiguousReferences =>
        if (nameParts.size > 1) {
          val attrRefs = ambiguousReferences.filter(refer => nameParts.head.contains(refer._1.qualifier.get) || refer._1.qualifier.get.contains(nameParts.head))
          if (! attrRefs.isEmpty) {
            Some(attrRefs.head._1)
          } else {
          ...
          val attrRefs = ambiguousReferences.filter(refer => nameParts.head.contains(refer._1.name) || refer._1.name.contains(nameParts.head))
          if (! attrRefs.isEmpty) {
            Some(attrRefs.head._1)
          } else {
            val referenceNames = ambiguousReferences.map(_._1).mkString(", ")
