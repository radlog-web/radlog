...
@ExpressionDescription(
  usage = "a _FUNC_ b - Returns FALSE if a equals b and true otherwise.")
case class NotEqualTo(left: Expression, right: Expression)
    extends BinaryComparison with NullIntolerant {
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    defineCodeGen(ctx, ev, (c1, c2) => s"!(${ctx.genEqual(left.dataType, c1, c2)})")
  }
}

@ExpressionDescription(
  usage = "a _FUNC_ b - Returns TRUE if a equals b and false otherwise.")
case class AttributeEqualTo(left: AttributeReference, right: AttributeReference)
    extends BinaryComparison with NullIntolerant {
...
@ExpressionDescription(
  usage = "a _FUNC_ b - Returns TRUE if a equals b and false otherwise.")
case class AttributeEqualToLiteral(left: AttributeReference, right: Literal)
    extends BinaryComparison with NullIntolerant {
