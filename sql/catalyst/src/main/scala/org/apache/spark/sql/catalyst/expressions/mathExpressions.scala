...
object Step{
  def step(v: Double): Int = if (Math.signum(v) < 0) 0 else 1
}

@ExpressionDescription(
  usage = "_FUNC_(x) - Returns the sign of x.")
case class Step(child: Expression) extends UnaryMathExpression(
  (v) => if (Math.signum(v) < 0) 0 else 1, "STEP") {
  override def dataType: DataType = IntegerType

  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    defineCodeGen(ctx, ev, c => s"org.apache.spark.sql.catalyst.expressions.Step.step($c)")
  }
}
