...
@ExpressionDescription(
  usage = "_FUNC_(a, b) - Returns the order preserving compression",
  extended = "> SELECT _FUNC_(10,3);\n 1")
case class Bopc(left: Expression, right: Expression) extends BinaryArithmetic with NullIntolerant {

  override def toString: String = s"opc($left, $right)"

  override def symbol: String = "opc"

  protected def checkTypesInternal(t: DataType) =
    TypeUtils.checkForNumericExpr(t, "opc")

  override def inputType: AbstractDataType = NumericType

  protected override def nullSafeEval(left: Any, right: Any) = opc(left, right)

  val mask = (1 << 16) - 1 // 16 indicates getting the last 15 bits of given Integer value

  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    nullSafeCodeGen(ctx, ev, (eval1, eval2) => {
      val shortHash = ctx.freshName("shortHash")
      val hashInt = ctx.freshName("hashInt")
      val emptS = ctx.freshName("emptS")
      dataType match {
        case dt: DecimalType =>
          s"""
            ${ctx.javaType(IntegerType)} $hashInt = ($eval1 + "" + $eval2).hashCode();
            ${ctx.javaType(IntegerType)} $shortHash = $hashInt & $mask;
            if ($shortHash < 0) {
              ${ev.value} = java.lang.Math.abs($shortHash);
            } else {
              ${ev.value} = $shortHash;
            }
          """
  ...
  // ${ev.value} = java.lang.Math.abs($shortHash);
  private def opc(a: Any, n: Any): Any = {
    val hc = (a.toString + n.toString).hashCode
    val r = hc >> 16
    if (r < 0) {Math.abs(r)} else r
  }
