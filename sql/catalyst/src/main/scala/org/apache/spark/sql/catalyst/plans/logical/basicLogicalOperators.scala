...
/**
 * A hint for the optimizer that we should cache the `child`and use shuffle hash join.
 */
...
case class WithRec(
    child: LogicalPlan,
    viewDefs: Map[String, (Seq[Expression], LogicalPlan)],
    source: String)
  extends UnaryNode {
  override def output: Seq[Attribute] = child.output

  override def toString: String = {
    viewDefs.map(e => s"${e._1}(${e._2._1.mkString(", ")})\n${e._2._2}").mkString("\n")
  }
}
