...
import org.apache.spark.sql.execution.{ProjectExec, SparkPlan, aggregate}
import org.apache.spark.sql.internal.SQLConf

case class ShuffleDistinct(conf: SQLConf) extends Rule[SparkPlan] {
              ...
              case exchange: Exchange => {
                exchange.withNewChildren {
                  exchange.children.map {
                    c => if (c.isInstanceOf[ProjectExec]) insertDistinctAggregate(c.asInstanceOf[ProjectExec]) else c
                  }
                }
              }
  ...
  def insertDistinctAggregate(child: ProjectExec): SparkPlan = {
