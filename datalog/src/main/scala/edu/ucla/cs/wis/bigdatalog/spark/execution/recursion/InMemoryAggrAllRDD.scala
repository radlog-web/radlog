...
class InMemoryAggrAllRDD(
  ...
  logInfo("Create InMemoryAggrAllRDD with # of partitions: " + numPartitions)
  ...
  override def computeOrReadCheckpoint(split: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    mAggr.aggrIterType match {
      case PrimitiveMSumAggrIterType => if (isNewAggr) new List2IntMCaseAggregationMapIterator(name, split.index, rowLength)
        else new Long2DoubleMCaseAggregationMapIterator(name, split.index, rowLength)
      case PrimitiveMCountAggrIterType => if (isNewAggr) new List2IntMCaseAggregationMapIterator(name, split.index, rowLength)
        else new Long2DoubleMCaseAggregationMapIterator(name, split.index, rowLength)
      case PrimitiveAggrIterType => if (isNewAggr) new List2IntMCaseAggregationMapIterator(name, split.index, rowLength)
        else new Long2DoubleMCaseAggregationMapIterator(name, split.index, rowLength)
      case _ =>
        new InMemoryAggregationMapIterator(name, split.index)
    }
  }
