 ...
 */
case class BroadcastHashJoinPackedExec(
  ...
  override def requiredChildDistribution: Seq[Distribution] = {
    // HACK: we want to broadcast the original relation so that it can be reused
    // similar to BroadcastNestedLoopJoinExec
    buildSide match {
      case BuildLeft =>
        BroadcastDistribution(PackedIdentityBroadcastMode(left.output)) :: UnspecifiedDistribution :: Nil
      case BuildRight =>
        UnspecifiedDistribution :: BroadcastDistribution(PackedIdentityBroadcastMode(right.output)) :: Nil
    }
  }
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    // HACK: broadcast the original relation instead of hashed relation
    val broadcastRelation = buildPlan.executeBroadcast[Array[Array[Byte]]]()
    val nFields = buildPlan.output.length

    streamedPlan.execute().mapPartitionsWithIndex { (index, streamedIter) =>
      // HACK: instead of broadcasting hashed relation, we broadcast the original relation,
      // and build the hashed relation on each worker (do it once and put it in dataKeeper)
      // Check [[ShuffledHashJoinExec]]
      val s1 = System.currentTimeMillis()
      val hashed = PartitionDataKeeper.instance().getOrBuildHashedRelation(broadcastRelation, buildKeys, nFields)
      logInfo(s"Get/Build HashedRelation (partition $index): ${System.currentTimeMillis() - s1} ms")

      val s2 = System.currentTimeMillis()
      val res = join(streamedIter, hashed, numOutputRows)
      logInfo(s"HashJoin (partition $index): ${System.currentTimeMillis() - s2} ms")
      res
    }
  }
   ...
   */
  private def prepareBroadcast(ctx: CodegenContext): (Boolean, String) = {
    // create a name for HashedRelation
    val broadcastRelation = buildPlan.executeBroadcast[Array[Array[Byte]]]()
    val broadcast = ctx.addReferenceObj("broadcast", broadcastRelation)
    val bks = ctx.addReferenceObj("bks", buildKeys)
    val nFields = ctx.addReferenceObj("nFields", buildPlan.output.length)
    val relationTerm = ctx.freshName("relation")
    val clsName = "org.apache.spark.sql.execution.joins.HashedRelation"
    ctx.addMutableState(clsName, relationTerm,
      s"""
         | $relationTerm = org.apache.spark.sql.datalog.PartitionDataKeeper.instance().getOrBuildHashedRelation($broadcast, $bks, $nFields);
       """.stripMargin)
    (false, relationTerm)
  }
  ...
  private def codegenInner(ctx: CodegenContext, input: Seq[ExprCode]): String = {
    val (keyIsUnique, relationTerm) = prepareBroadcast(ctx)
  ...
  private def codegenAnti(ctx: CodegenContext, input: Seq[ExprCode]): String = {
    val (keyIsUnique, relationTerm) = prepareBroadcast(ctx)
    val (keyEv, anyNull) = genStreamSideJoinKey(ctx, input)
    val (matched, checkCondition, _) = getJoinCondition(ctx, input, keyIsUnique)
    val numOutput = metricTerm(ctx, "numOutputRows")
