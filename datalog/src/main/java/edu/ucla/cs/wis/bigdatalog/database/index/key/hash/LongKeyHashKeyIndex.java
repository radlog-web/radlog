...
public class LongKeyHashKeyIndex 
	extends LongKeyKeyIndex 
	implements MemorySize, Serializable {
	private static Logger logger = LoggerFactory.getLogger(LongKeyHashKeyIndex.class.toString());
	...
	protected LongKeyBucket[][]			directory;
	...
	public LongKeyHashKeyIndex(Relation relation, int[] keyColumns, double splitPolicy, 
	...
	private void initialize() {
		...
		this.directory 					= new LongKeyBucket[this.directorySize][];
		...
		for (int i = 0; i < numberOfSegments; i++)
			this.directory[i] = new LongKeyBucket[this.segmentSize];
		
		this.bytesPerKey = this.getKeySize();

		// initialize the buckets
		for (int i = 0; i < this.numberOfAllocatedBuckets; i++)
			this.directory[this.getSegmentPositionWithinDirectory(i)][this.getBucketPositionWithinSegment(i)] = new LongKeyBucket();
	...
	private LongKeyBucket getBucket(int bucketNumber) {
		return this.directory[bucketNumber / this.segmentSize][bucketNumber % this.segmentSize];
	}
	
	private void setBucket(int bucketNumber, LongKeyBucket bucket) {
	...
	private void split() {
		if ((this.nextBucketToSplit + this.upperBoundNumberOfBuckets) < (this.directorySize * this.segmentSize)) {					
			LongKeyBucket bucketToSplit = this.getBucket(this.nextBucketToSplit);
			...
			if ((newBucketNumber % this.segmentSize) == 0)
				this.directory[newBucketNumber / this.segmentSize] = new LongKeyBucket[segmentSize];
						
			LongKeyBucket newBucket = new LongKeyBucket();
			...
			for (int i = 0; i < bucketToSplit.getNumberOfKeys(); i++) {
				...
				if (bucketNumber == newBucketNumber) {
					...
					newBucket.putFromSplit(keys[i], hashes[i]);
				}
			}
