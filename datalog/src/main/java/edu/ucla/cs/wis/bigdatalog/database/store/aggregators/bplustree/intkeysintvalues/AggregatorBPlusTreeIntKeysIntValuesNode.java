...
public class AggregatorBPlusTreeIntKeysIntValuesNode 
	extends AggregatorBPlusTreeNode<AggregatorBPlusTreeIntKeysIntValuesPage, AggregatorBPlusTreeIntKeysIntValuesLeaf> 
	implements AggregatorBPlusTreeIntKeysIntValuesPage, Serializable {
	private static final long serialVersionUID = 1L;

	public int[] keys;
	
	public AggregatorBPlusTreeIntKeysIntValuesNode() { super(); }
	
	public AggregatorBPlusTreeIntKeysIntValuesNode(int nodeSize, AggregateFunctionType aggregateType) {
		super(nodeSize, 4, aggregateType);		
		//this.aggregator = aggregator;
		
		this.highWaterMark = 0;
		this.keys = new int[this.numberOfKeys]; // M-1 keys
		this.children = new AggregatorBPlusTreeIntKeysIntValuesPage[this.getBranchingFactor()]; // M children
	...
	@Override
	public void insert(int key, int data, AggregatorBPlusTreeIntKeysIntValuesInsertResult result) {
		int insertAt = 0;
	...
	private AggregatorBPlusTreeIntKeysIntValuesPage split() {
		AggregatorBPlusTreeIntKeysIntValuesNode rightNode 
			= new AggregatorBPlusTreeIntKeysIntValuesNode(this.nodeSize, this.aggregateType);
