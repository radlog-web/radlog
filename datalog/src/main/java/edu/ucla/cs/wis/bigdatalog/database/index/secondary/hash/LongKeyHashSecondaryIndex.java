...
*/
public class LongKeyHashSecondaryIndex 
	extends LongKeySecondaryIndex<AddressedTupleStore> 
	implements Serializable {
	private static Logger logger = LoggerFactory.getLogger(LongKeyHashSecondaryIndex.class.toString());
	
	private static final long serialVersionUID = 1L;
	
	protected int							numberOfInitialBuckets;
	protected LongKeyBucketOfTupleAddresses[][]	directory;
	...
	public LongKeyHashSecondaryIndex(Relation<AddressedTuple> relation, int[] indexedColumns, double splitPolicy, 
	...
	private void initialize() {	
		this.directory 					= new LongKeyBucketOfTupleAddresses[this.directorySize][];
		...
		for (int i = 0; i < numberOfSegments; i++)
			this.directory[i] = new LongKeyBucketOfTupleAddresses[segmentSize];
		
		// initialize the buckets
		for (int i = 0; i < this.numberOfAllocatedBuckets; i++)
			this.directory[this.getSegmentPositionWithinDirectory(i)][this.getBucketPositionWithinSegment(i)] = new LongKeyBucketOfTupleAddresses();
	}
	...
	private static long hash(long key) {
		...
		return MurmurHash.hash(ByteArrayHelper.getLongAsBytes(key));
	}
	...
	private LongKeyBucketOfTupleAddresses getBucket(int bucketNumber) {
		return this.directory[bucketNumber / this.segmentSize][bucketNumber % this.segmentSize];
	}
	
	private void setBucket(int bucketNumber, LongKeyBucketOfTupleAddresses bucket) {
	...
	private void split() {
		if ((this.nextBucketToSplit + this.upperBoundNumberOfBuckets) < (this.directorySize * this.segmentSize)) {					
			LongKeyBucketOfTupleAddresses bucketToSplit = this.getBucket(this.nextBucketToSplit);
			...
			if ((newBucketNumber % this.segmentSize) == 0)
				this.directory[newBucketNumber / this.segmentSize] = new LongKeyBucketOfTupleAddresses[segmentSize];
						
			LongKeyBucketOfTupleAddresses newBucket = new LongKeyBucketOfTupleAddresses();
			...
			int bucketNumber;
			long[] keys = bucketToSplit.getKeys();
