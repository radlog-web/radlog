...
public class TupleBPlusTreeUniqueStoreIntKeysRangeSearchResultScanCursor 
	extends TupleBPlusTreeUniqueStoreRangeSearchResultScanCursor<BPlusTreeIntKeysByteValuesLeaf, Integer, int[]> {
	
	public TupleBPlusTreeUniqueStoreIntKeysRangeSearchResultScanCursor(Relation relation) {
		super(relation);
	}
/*
	@Override
	public void initialize(BPlusTreeLeaf<?> startLeaf, int startIndex, RangeSearchKeys<?> keys) {
		this.tupleStore = (BPlusTreeTupleStore) relation.getTupleStore();
		this.storageStructure = ((TupleBPlusTreeUniqueStore)relation.getTupleStore()).storageStructure;
		this.currentLeaf = (BPlusTreeIntKeysByteValuesLeaf) startLeaf;
		this.keyIndex = startIndex;
		this.searchKeys = keys;
		this.endKey = (int) keys.endKey;
