...
public class AggregatorBPlusTreeIntKeysFloatValuesNode 
	extends AggregatorBPlusTreeNode<AggregatorBPlusTreeIntKeysFloatValuesPage, AggregatorBPlusTreeIntKeysFloatValuesLeaf> 
	implements AggregatorBPlusTreeIntKeysFloatValuesPage, Serializable {
	private static final long serialVersionUID = 1L;

	protected int[] keys;

	public AggregatorBPlusTreeIntKeysFloatValuesNode() { super(); }
	
	public AggregatorBPlusTreeIntKeysFloatValuesNode(int nodeSize, AggregateFunctionType aggregateType) {
		super(nodeSize, 4, aggregateType);
		
		this.highWaterMark = 0;
		this.keys = new int[this.numberOfKeys]; // M-1 keys
		this.children = new AggregatorBPlusTreeIntKeysFloatValuesPage[this.getBranchingFactor()]; // M children
	...
	@Override
	public void insert(int key, double data, AggregatorBPlusTreeIntKeysFloatValuesInsertResult result) {
		int insertAt = 0;

		if (this.children[insertAt] == null) {
			this.children[insertAt] = new AggregatorBPlusTreeIntKeysFloatValuesLeaf(this.nodeSize, this.aggregateType);			
			this.highWaterMark++;
		} else {
		...
		if ((insertAt + 1) < this.highWaterMark)
			...
			//for (int j = this.highWaterMark; j > insertAt; j--)this.children[j] = this.children[j - 1];
			// shift right for insertion to the left
	...
	private AggregatorBPlusTreeIntKeysFloatValuesPage split() {
		AggregatorBPlusTreeIntKeysFloatValuesNode rightNode = new AggregatorBPlusTreeIntKeysFloatValuesNode(this.nodeSize, this.aggregateType);
