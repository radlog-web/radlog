...
public class LongKeyBucketOfTupleAddresses implements MemorySize, Serializable {
	...
	public LongKeyBucketOfTupleAddresses() {
		this.initialize();
	}
	
	private void initialize() {
		this.highWaterMark = 0;
		this.keys = new long[INITIAL_NUMBER_OF_KEYS];	
	...
	public void setEntries(long[] keys, long[] hashes, TupleAddressArray[] entries) { 	
	...
	public boolean put(long key, long hash, int address) {
	...
	public void putFromSplit(long key, long hash, TupleAddressArray entries) {
		if (this.isFull())
			this.grow();
	...
	public boolean remove(long key, int address) {
		for (int i = 0; i < this.highWaterMark; i++) {
			if (this.keys[i] == key) {
