...
public class MurmurHash {
    ...
    // APS 11/22/2013 added
    public static long hash(byte[] key) {
    	ByteBuffer b = ByteBuffer.wrap(key);
    	// we only take the first hash, even though there are two.
    	return MurmurHash.hash3_x64_128(b, b.position(), b.remaining(), 0L)[0]; 
    	//return hash[0];// & 0x7fffffff;
    }
    
    public static long hash(byte[] key, int i) {
    	ByteBuffer b = ByteBuffer.wrap(key);
    	// we only take the first hash, even though there are two.
    	return MurmurHash.hash3_x64_128(b, b.position(), b.remaining(), 0L)[i]; 
    	//return hash[0];// & 0x7fffffff;
    }
}
