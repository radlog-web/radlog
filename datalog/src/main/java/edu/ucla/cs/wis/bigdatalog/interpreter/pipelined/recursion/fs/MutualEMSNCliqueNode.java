...
public class MutualEMSNCliqueNode 
	extends EMSNCliqueNode 
	implements IMutualClique {
	
	public MutualEMSNCliqueNode(String predicateName, NodeArguments args, Binding binding, 
	...
	public int getAnyTuple(Cursor readCursor, CliqueBaseNode clique, Tuple tuple) {
		...
		if (DEBUG) {
			if (getStatus > 0)
				this.logTrace("Exiting getAnyTuple for {} with tuple {} with SUCCESS (status = {})", this.toStringNode(), tuple, getStatus);
			else
	...
	public boolean isFixedpointReached() {
		this.logError("MutualFSCliqueNode4.isFixedpointReached() should not be called on Mutual FS Cliques");
		throw new InterpreterException("MutualFSCliqueNode4.isFixedpointReached() should not be called on Mutual FS Cliques");
	}
	...
	/*
	@Override
	public MutualEMSNCliqueNode copy(ProgramContext programContext) {
		if (programContext.getCliqueMapping().containsKey(this))
			return (MutualEMSNCliqueNode) programContext.getCliqueMapping().get(this);
		
		MutualEMSNCliqueNode copy = new MutualEMSNCliqueNode(new String(this.predicateName), 
