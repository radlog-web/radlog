...
public class AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf 
	extends BPlusTreeLeaf<AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf> 
	implements AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesPage, Serializable { 
	...
	public AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf() { super(); }
	
	public AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf(int nodeSize, DataType totalValueType, 
		...
		if ((aggregateType != AggregateFunctionType.FSCNT) && (aggregateType != AggregateFunctionType.FSSUM))
			throw new DatabaseException("AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf should only be used with fscnt or fssum.");
	...
	@Override
	public void insert(long key, DbTypeBase subKey, DbNumericType newPartialValue, 
		...
		for (i = 0; i < this.highWaterMark; i++) {
			...
			if (this.keys[i] == key) {
				...
				DbNumericType oldPartialValue = (DbNumericType) group.put(subKey, newPartialValue);
			 	if (oldPartialValue == null) {
			 		//System.out.println("no old value");
			 	} else if (newPartialValue.equals(oldPartialValue)) {
					//System.out.println("\\\\\\new value "+newPartialValue+" equal to " + oldPartialValue+"\\\\\\\"");
					result.newPage = null;
					...
					//System.out.println("\\\\\\new value "+newPartialValue+" not greater than " + oldPartialValue+"\\\\\\\"");
					//System.out.println("new value "+newPartialValue+" not greater than " + oldPartialValue);
					result.newPage = null;
					result.status = AggregatorInsertStatus.FAIL;
					result.newTotalResult = null;
					return;
				}
				// last case is if we have greater than new partial value
	...
	private AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesPage split() {
		AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf rightLeaf = 
				new AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf(this.nodeSize, this.totalValueType, 
