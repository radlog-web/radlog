...
public class AggregatorBPlusTreeLongKeysDbTypeDbKeyValues 
	extends AggregatorBPlusTreeStoreStructure<AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesPage, 
		AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf, AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesNode> 
	implements ChangeTrackingStore<Long>, RangeSearchableStorageStructure<Long>, Serializable {
	private static final long serialVersionUID = 1L;

	protected AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesResult insertResult;
	protected AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesGetResult getResult;
	protected AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesGetRangeResult getRangeResult;
	
	public AggregatorBPlusTreeLongKeysDbTypeDbKeyValues() { super(); }
	
	public AggregatorBPlusTreeLongKeysDbTypeDbKeyValues(int nodeSize, int[] keyColumns, DataType[] keyColumnTypes, 
		...
		this.insertResult = new AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesResult();
		this.getResult = new AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesGetResult(this.valueColumnTypes[0]);
		this.getRangeResult = new AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesGetRangeResult();
		this.initialize();
	}

	@Override
	protected AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesPage allocatePage() {
		if (this.rootNode == null)
			return new AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesLeaf(this.nodeSize, this.valueColumnTypes[0], 
					this.valueColumnTypes[1], this.aggregateInfos[0].aggregateType);

		return new AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesNode(this.nodeSize, this.valueColumnTypes[0], 
				this.valueColumnTypes[1], this.aggregateInfos[0].aggregateType);
	}
	...
	public void insert(Tuple tuple, AggregatorResult result) {
		...
		AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesNode newRoot = (AggregatorBPlusTreeLongKeysDbTypeDbKeyValuesNode)this.allocatePage();
		...
		if (this.getResult.success) {
			if (this.keyColumns.length == 1) {	
				...
				if (this.keyColumnTypes[1] == DataType.INT)
					tuple.columns[this.keyColumns[1]] = DbInteger.create(key.intValue());
				else
					tuple.columns[this.keyColumns[1]] = DbString.load(key.intValue(), this.typeManager);
			}
			tuple.columns[this.valueColumns[0]] = this.getResult.value;
			return 1;
		}
	...
	public void getTuple(Long startKey, Long endKey, RangeSearchResult result) {
		this.rootNode.get(startKey, endKey, this.getRangeResult);

		result.success = this.getRangeResult.success;
		if (this.getRangeResult.success) {
			result.index = this.getRangeResult.index;
			result.leaf = this.getRangeResult.leaf;
			result.success = true;
			return;
		}
	...
	public int loadTuple(long key, DbTypeBase value1, DbTypeBase value2, Tuple tuple) {
		if (this.keyColumns.length == 1) {	
			tuple.columns[this.keyColumns[0]] = DbLong.create(key);				
		} else {
			if (this.keyColumnTypes[0] == DataType.INT)
