...
*/
public class GeneralKeyHashSecondaryIndex 
	extends GeneralKeySecondaryIndex<AddressedTupleStore> 
	implements Serializable {
	private static Logger logger = LoggerFactory.getLogger(GeneralKeyHashSecondaryIndex.class.toString());
	
	private static final long serialVersionUID = 1L;
	
	protected final int						numberOfInitialBuckets;
	protected GeneralKeyBucketOfTupleAddresses[][]	directory;
	...
	public GeneralKeyHashSecondaryIndex(Relation<AddressedTuple> relation, int[] indexedColumns, 
			double splitPolicy, int directorySize, int segmentSize, int numberOfInitialBuckets) {
	...
	private void initialize() {	
		this.directory 					= new GeneralKeyBucketOfTupleAddresses[this.directorySize][];
		...
		for (int i = 0; i < numberOfSegments; i++)
			this.directory[i] = new GeneralKeyBucketOfTupleAddresses[this.segmentSize];
		
		// initialize the buckets
		for (int i = 0; i < this.numberOfAllocatedBuckets; i++)
			this.directory[this.getSegmentPositionWithinDirectory(i)][this.getBucketPositionWithinSegment(i)] = new GeneralKeyBucketOfTupleAddresses(this.bytesPerKey);
	}
	...
	public boolean doPut(byte[] key, AddressedTuple tuple) {
		...
		int bucketNumber = this.getBucketNumber(hash);

		// returns false if key already in bucket
		if (this.getBucket(bucketNumber).put(key, hash, tuple.address)) {
	...
	public AddressedTuple doGet(byte[] key, Tuple tuple) {
		...
		if (entry != null) {
			...
			// check each address to find matching tuple
	...
	protected int[] doGetSimilar(byte[] key) {
		long hash = hash(key);
		int bucketNumber = this.getBucketNumber(hash);
		TupleAddressArray entry = this.getBucket(bucketNumber).get(key, hash);
		if (entry != null)
	...
	private GeneralKeyBucketOfTupleAddresses getBucket(int bucketNumber) {
		return this.directory[bucketNumber / this.segmentSize][bucketNumber % this.segmentSize];
	}
	
	private void setBucket(int bucketNumber, GeneralKeyBucketOfTupleAddresses bucket) {
	...
	private void split() {
		if ((this.nextBucketToSplit + this.upperBoundNumberOfBuckets) < (this.directorySize * this.segmentSize)) {					
			GeneralKeyBucketOfTupleAddresses bucketToSplit = this.getBucket(this.nextBucketToSplit);
			...
			if ((newBucketNumber % this.segmentSize) == 0)
				this.directory[newBucketNumber / this.segmentSize] = new GeneralKeyBucketOfTupleAddresses[segmentSize];
						
			GeneralKeyBucketOfTupleAddresses newBucket = new GeneralKeyBucketOfTupleAddresses(this.bytesPerKey);
			...
			for (int i = 0; i < bucketToSplit.getNumberOfKeys(); i++) {
				...
				if (bucketNumber == newBucketNumber) {
					...
					newBucket.putFromSplit(Arrays.copyOfRange(keys, i * this.bytesPerKey, (i + 1) * this.bytesPerKey), hashes[i], entries[i]);
				}
			}
