...
import edu.ucla.cs.wis.bigdatalog.spark.runner.DatalogRunner

/**
 * Use `build/sbt "project datalog" test:compile` to compile source and test classes in datalog project.
 */
class DatalogTestSuite extends TestSuite {
  ...
  val testCases = Seq(
    ...
    ET("cc_d", Seq("-arc=testdata/cc.csv")),
  ...
  run(new DatalogRunner(), testCases)
