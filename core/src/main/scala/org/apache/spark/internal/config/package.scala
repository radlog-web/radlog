...
package object config {
  ...
  // Note: This is a SQL config but needs to be in core because it chooses the SessionState type
  private[spark] val SESSION_STATE = ConfigBuilder("spark.sql.sessionState")
    .doc("This config decides if [[RaSQLSessionState]] will be used.")
    .stringConf
    .checkValues(Set("rasql", "spark"))
    .createWithDefault("rasql")
