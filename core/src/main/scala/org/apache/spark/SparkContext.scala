...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
   ...
   * Similar to runApproximateJob
   *
   * victor: APS
   */
  def runFixedPointJob[T](rdd: RDD[T]): Unit = {
    assertNotStopped()
    val callSite = getCallSite
    dagScheduler.runFixedPointJob(rdd, callSite, localProperties.get)
    // TODO: set the correct return value, the input rdd is fully executed at this time
  }
