...
// victor
private[spark] class ZippedPartitionsRDD2WithIndex[A: ClassTag, B: ClassTag, V: ClassTag](
    sc: SparkContext,
    var f: (Int, Int, Iterator[A], Iterator[B]) => Iterator[V],
    var rdd1: RDD[A],
    var rdd2: RDD[B],
    preservesPartitioning: Boolean = false)
  extends ZippedPartitionsBaseRDD[V](sc, List(rdd1, rdd2), preservesPartitioning) {
  ...
  override def compute(s: Partition, context: TaskContext): Iterator[V] = {
    val partitions = s.asInstanceOf[ZippedPartitionsPartition].partitions
    val idx1 = partitions(0).index
    val idx2 = partitions(1).index
    f(idx1, idx2, rdd1.iterator(partitions(0), context), rdd2.iterator(partitions(1), context))
  }
