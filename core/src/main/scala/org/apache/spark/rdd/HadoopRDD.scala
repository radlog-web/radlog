...
import org.apache.spark.scheduler.{HDFSCacheTaskLocation, Hosts, HostTaskLocation}
...
class HadoopRDD[K, V](
  ...
  override def getPreferredLocations(split: Partition): Seq[String] = {
    ...
    val res: Seq[String] = locs.getOrElse(hsplit.getLocations.filter(_ != "localhost"))
    // TODO: hack for locality
    if (res.nonEmpty) {
      val pinned = res.map(Hosts.toPinnedHostIP)
      logDebug(s"Hadoop Split ${split.index} pinned location size: ${pinned.length}")
      pinned
    } else {
      res
    }
