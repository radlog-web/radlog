...
import org.apache.spark.scheduler.fixedpoint.{FixedPointJobDefinition, FixedPointJobListener, FixedPointResultStage}
...
class DAGScheduler(
    ...
    shuffleToMapStage.get(shuffleDep.shuffleId) match {
      ...
      case None =>
        ...
        getAncestorShuffleDependencies(shuffleDep.rdd).foreach { dep =>
          // logInfo("GetAncestorShuffleDependencies: shuffleId " + dep.shuffleId)
  ...
  private def newFixedPointResultStage(
    ...
    val (parentStages: List[Stage], id: Int) = getParentStagesAndId(rdd, jobId)
    logInfo("New FixedPointResultStage: " + id + "\n\n")
  ...
  private def newOrUsedShuffleStage(
      shuffleDep: ShuffleDependency[_, _, _],
      firstJobId: Int): ShuffleMapStage = {
    logInfo("NewOrUsedShuffleStage: shuffleId " + shuffleDep.shuffleId)
        ...
        case (stageId, stage) =>
          ...
          if (!jobSet.contains(job.jobId)) {
            ...
            if (jobSet.isEmpty) { // no other job needs this stage
              // victor - we don't remove stage here, as it will be needed for later iterations
              // TODO: remove all stages after fixedpoint reached
              // removeStage(stageId)
            }
          }
      }
  ...
  /**
   * victor: bigdatalog
   * code logic similar to runApproximateJob
   */
  def runFixedPointJob[T, U](
      rdd: RDD[T],
      callSite: CallSite,
      properties: Properties): Unit = {
    val start = System.nanoTime

    val listener = new FixedPointJobListener(rdd)
    val func2 = (tx: TaskContext, iter: Iterator[_]) => false
    val partitions = (0 until rdd.partitions.length).toArray
    val jobId = nextJobId.getAndIncrement()
    eventProcessLoop.post(JobSubmitted(
      jobId, rdd, func2, partitions, callSite, listener, SerializationUtils.clone(properties)))
    val result = listener.awaitResult()    // Will throw an exception if the job fails
    result match {
      case JobSucceeded =>
        logInfo("Fixed Point Job %d finished: %s, took %f s".format
        (jobId, callSite.shortForm, (System.nanoTime - start) / 1e9))
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      // victor: bigdatalog
      finalStage = if (listener.isInstanceOf[FixedPointJobListener[_]]) {
  ...
  private[scheduler] def handleMapStageSubmitted(jobId: Int,
      ...
      listener: JobListener,
      properties: Properties) {
    logInfo("HandleMapStageSubmitted: shuffleId " + dependency.shuffleId)
      ...
      stage match {
        ...
        case stage: FixedPointResultStage =>
          ...
          partitionsToCompute.map { id =>
            ...
            val constructor = Utils.classForName("org.apache.spark.sql.datalog.FixedPointResultTask").getConstructors()(0)
            val args = Array(Int.box(stage.id), Int.box(stage.latestInfo.attemptId), taskBinary, part, locs, Int.box(id), properties, stage.latestInfo.taskMetrics)
            constructor.newInstance(args: _*).asInstanceOf[Task[_]]
          }
...
//  }
  ...
  private def markStageAsFinished(stage: Stage, errorMessage: Option[String] = None): Unit = {
    ...
    if (errorMessage.isEmpty) {
      ...
      println("%s: %s ms".format(stage, (serviceTime.toDouble * 1000).toInt.toString))
