...
private[spark] class TaskSetManager(
  ...
  private def addPendingTask(index: Int) {
    var isPinned = false
    // victor: we pin a task to a specific host
    for (loc <- tasks(index).preferredLocations) {
      if (loc.isInstanceOf[PinnedHostTaskLocation]) {
        val t = tasks(index)
        logDebug(s"$t is pinned to host $loc")
        pendingTasksForHost.getOrElseUpdate(loc.host, new ArrayBuffer) += index
        isPinned = true
      }
    }
    // if pinned, we return here thus tasks will not be added to other nodes
    if (isPinned) return
  ...
  private def getLocalityWait(level: TaskLocality.TaskLocality): Long = {
    val defaultWait = conf.get("spark.locality.wait", "0s")
