...
/**
 * victor: A pinned task on a given host
 */
private [spark] case class PinnedHostTaskLocation(override val host: String) extends TaskLocation {
  override def toString: String = host
}
...
object TaskLocation {
  ...
  def apply(str: String): TaskLocation = {
    // victor
    if (str.startsWith(pinnedHostTag)) {
      return PinnedHostTaskLocation(str.stripPrefix(pinnedHostTag))
    }
