...
private[spark] class Executor(
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      // victor: we replace "new TaskMemoryManager(env.memoryManager, taskId)"
      // by "env.allTasksMemoryManager"
      // because we want all tasks to share the same memoryManager
      val taskMemoryManager = env.allTasksMemoryManager
      ...
      try {
        ...
        val value = try {
          ...
          // victor: we do not free memory as we have shared data structures
